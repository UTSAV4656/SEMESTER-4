...
END TRY
BEGIN CATCH
		PRINT 'ERROR OCCOURED : DIVIDE BY ZERO'
END CATCH
...
BEGIN
    BEGIN TRY
        DECLARE @NO1 INT = CAST(@N1 AS INT);
        DECLARE @NO2 INT = CAST(@N2 AS INT);
        PRINT 'Sum is: ' + CAST(@NO1 + @NO2 AS VARCHAR(50));
    END TRY
    BEGIN CATCH
...
EXEC PR_Calculation_SumWithErrorHandling A,2
